...
/*
    아이콘 사이즈 조절부
    그러나 superclass 이므로 직접 수정은 삼가야
    2020.02.24. staticccast
*/
Window_Base._iconWidth  = 32;       //원본은 각각 32
...
Window_Base.prototype.lineHeight = function() {
    return 36;
    /*
    lineHeight가 Window_Base에서 고정되어있으므로
    lineHeight -> Selectable의 ItemHeight -> RectHeight
    로 계승 -> 아이템창 한 항목의 높이도 36 고정값.
    2020.02.24. staticccast
    */
};
...
Window_Base.prototype.standardFontFace = function() {
    ...
    /*
    메뉴 폰트 변경 - 멀티플랫폼이라면 추후에 글씨체별로 테스트 필요할 것.
    GameFont를 추가 - gamefont.css에서 조절 - 현재 SeoulNamsanM.ttf
    2020.06.03. staticccast
    */
    } else {
        return 'GameFont';
    }
...
Window_Base.prototype.drawTextEx2 = function(text, x, y, fontSizeForItemHelpWindow) {
/*
    아이템 설명창 텍스트 한정으로 쓰일 함수
    항상 고정 폰트 사이즈로 출력
    
    동명의 함수와 같은 작동방식이지만
    (따라서 \C 등이 전부 정상 작동)
    인자를 하나 더 받게 새로 만듦
    2020.06.05. staticccast
*/
    if (text) {
        var textState = { index: 0, x: x, y: y, left: x };
        textState.text = this.convertEscapeCharacters(text);
        textState.height = this.calcTextHeight(textState, false);
        this.contents.fontSize = fontSizeForItemHelpWindow;
        while (textState.index < textState.text.length) {
            this.processCharacter(textState);
        }
        this.resetFontSettings();
...
//터치나 클릭 시 좌표 알아내는 기능 2020.01.06. staticccast
...
/*
    아이템창에서 한칸 크기 조절
    Selectable 클래스이므로 다른 곳에도 관여
    따라서 직접 수정하지 말 것
    ItemList에서만 사이즈 바꿀 방법 찾아야
    2020.02.24. staticccast
*/
Window_Selectable.prototype.itemRect = function(index) {
    ...
    rect.height = this.itemHeight(); //this.lineHeight를 반환
...
Window_Command.prototype.maxItems = function() {
    ...
    //return this._list.length;
    /*
    Window_MenuCommand.prototype.initialize (line 1582) 에서

    Menu_Command를 windowHeight() 활용해 화면 가운데 정렬하면
    .length 값이 undefined 라는 에러 발생
    함수 호출 순서에 따른 에러로 추정
    ( 창을 만들면서 높이값이 측정되므로
        만들기 전에 접근해 위치를 정하려니 아직 값이 undefined )
    항목 개수는 미리 알 수 있으므로 하드코딩해서 해결
    2020.01.06. staticccast

    superclass이므로 여기를 수정하면 addCommand 관련 함수가 모두 오작동
    Menu_Command의 동작은 하위 클래스에서 수정할 것
    다시 원래 코드로 복구
    2020.01.07. staticccast
    */
};
...
/*
    아이템 설명 창 크기/위치 조절
    다른 클래스에 영향을 미치지 않으므로 직접 수정해도 무방할 것
*/

Window_Help.prototype.initialize = function(numLines) {
    var width = 500;
    var height = 96;
    Window_Base.prototype.initialize.call(this,(Graphics.boxWidth - width)/2, 36, width, height);
    //this.contents.fontSize = 19;
    this._text = '';
};

/*
Window_Base.fontSize 와
this.contents.fontSize 의 결과가 다르다
이유가 몰까
2020.06.03. staticccast
*/
...
Window_Help.prototype.setText = function(text) {
    ...
    this.contents.fontSize = 19; // 줄간격 조절 위해 임시방편, 폰트 크기는 변하지 않음. 2020.06.03. staticccast
...
Window_Help.prototype.refresh = function() {
    this.contents.clear();
    this.drawTextEx2(this._text, this.textPadding(), 0, 23);
};
...
Window_MenuCommand.prototype.initialize = function(x, y) {
    ...
    newX = (Graphics.boxWidth - this.windowWidth()) / 2;
    newY = (Graphics.boxHeight - this.fittingHeight(2)) / 2;
    // 메뉴(C 키 - 옵션, 게임종료)창을 화면 가운데에 위치시킴 2020.01.06. staticccast
    // 괄호 안은 메뉴의 항목 개수
    
    Window_Command.prototype.initialize.call(this, newX, newY);
    this.selectLast();
};
...
// 본래는 좌측 메뉴, 현재는 C 키 메뉴(옵션, 게임종료)
// MenuStatus (우측 상태 메뉴)에는 이 함수를 사용하지 않고 240으로 하드코딩 되어있으므로 수정 시엔 주의
// 2020.01.06. staticccast
...
/*
Item/Skill창 superclass 수정하기보다
이쪽을 수정하는 것이 더 안전할 것
2020.01.15 staticccast
*/
...
// 아이템 카테고리 창 크기 조절 2020.01.06. staticccast
Window_ItemCategory.prototype.initialize = function() {
    Window_HorzCommand.prototype.initialize.call(this, (Graphics.boxWidth-500)/2, 0);
};
...
Window_ItemCategory.prototype.makeCommandList = function() {
    //this.addCommand(TextManager.weapon,  'weapon');  //무기 삭제 및 핵심 아이템 부활 2020.05.29 louischoi
...
1.  무기, 아이템 순으로 배치 바꿈
    ...
    게임 내 아이템을 장착할 수 있게 하기 위함
    기록은 사용 아이템 -> 커먼 이벤트 식으로 구현

2.  방어구, 핵심 아이템 카테고리 삭제

    //this.addCommand(TextManager.armor,   'armor', false);
...
/*
    기존의 무기 시스템을 -> 손에 든 아이템 으로 이용하기 위해
    수정할 부분
    isEnabled, canEquip, canUse, changePaintOpacity 부분 자세히 참조
    2020.01.12. staticccast

    isCurrentItemEnabled()는 isEnabled를 호출
    isEnabled는 canUse()를 호출
    canUse()는 canUse(this.item())을 호출
    2020.01.17. staticccast
...
Window_SavefileList.prototype.drawItem = function(index) {
    ...
    if(this._mode === 'save' && id == 1) valid = 0;
    /*
        자동저장 슬롯 '불러오기 시에만' Opacity 어둡게 조절
        valid 변수는 외형에만(Opacity)관여, 저장 금지와 별개

        drawContents 전후에 valid를 조절해
        2020.05.30. staticccast
        valid == 0 일때에 주인공 캐릭터가 그려지지 않는 버그 수정
    */
    ...
    if (info) {
        ...
        if(id == 1) valid = 1; //2020.05.30. staticccast
...
//  저장/로드 텍스트 수정 2020.05.30. staticccast
Window_SavefileList.prototype.drawFileId = function(id, x, y) {
    if(id > 1) this.drawText(TextManager.file + ' ' + (id - 1), x, y, 180);
    else
    {
        this.drawText(TextManager.autoSaveSlot, x, y, 180);
    }
};
...
/*
    Window_ScrollText는 게임 내 '컴퓨터 메모장 열람 기능'을 구현하고자
    전체적으로 수정이 많이 가해졌음
    그림도 포함시키거나, 메모장 디자인을 삽입하려면 추후 수정이 더 필요할 것
    2020.03.25. staticccast
 */
...
Window_ScrollText.prototype.initialize = function() {
    ...
    Window_Base.prototype.initialize.call(this, 60, 50, width-120, height-100);
    //메모장 스크롤 텍스트 기능 : 창 크기/위치 조절 2020.05.08. staticccast
    this.opacity = 0;
    this.hide();
    this._text = '';
...
Window_ScrollText.prototype.refresh = function() {
    ...
    this.createContents();
    this.origin.y = 0;   // 원본 this.origin.y = -this.height;
...
Window_ScrollText.prototype.updateMessage = function() {
    if(TouchInput.wheelY > 0 && this.origin.y < (this.contents.height - Graphics.boxHeight + 150))
    {
        this.origin.y += 50;
        
    }
    else if(TouchInput.wheelY < 0 && 0 < this.origin.y)
    {
        this.origin.y -= 50;
    }
    /*  원본
...
Window_TitleCommand.prototype.makeCommandList = function() {
    ...
    this.addCommand(TextManager.gameEnd, 'gameEndFreal');
    // 타이틀에서 게임 종료 가능하게 2020.01.07. staticccast
};
...
Window_GameEnd.prototype.makeCommandList = function() {
    ...
    // 실제 게임종료 기능 추가
    // 2020.01.06. staticccast
    this.addCommand(TextManager.gameEndFrealFreal, 'gameEndFreal');
