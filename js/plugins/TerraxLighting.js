...
(function() {
			...
			if ($gameVariables.GetStopScript() == false) {
				...
				if ($gameVariables.GetScriptActive() == true && $gameMap.mapId() >= 0) {
					...
					if (darkenscreen == true) {
						...
						//원본 var y1 = (ph / 2) + ( (py - dy) * ph);
						var y1 = (ph / 2) + ( (py - dy) * ph) - 20; //staticccast 플레이어 광원을 키에 맞게 위치 조정
						var paralax = false;
						...
						if (iplayer_radius > 0) {
							if (playerflashlight == true) {
								this._maskBitmap.radialgradientFillRect2(x1, y1, 20, iplayer_radius, playercolor, '#000000', flashlightlength, flashlightwidth);
							}
								...
								|| note_command == "freelight") {
									...
									if (light_radius >= 0) {
										...
										if (state == true) {
											...
											if (flashlight == true) {
												this._maskBitmap.radialgradientFillRect2(lx1, ly1, 0, light_radius, colorvalue, '#000000', flashlength, flashwidth);
											} else if (note_command == "freelight"){
												this._maskBitmap.FillShape([lx1,ly1], [[0,-20],[40,-20],[100,200],[-60,200]], 0,0,0);
											} else {
	...
	Bitmap.prototype.FillShape = function(cntr, points, color1, color2, flicker){
		//자유 형태 빛 함수 2020.07.24. staticccast
		var isValidColor = /(^#[0-9A-F]{6}$)|(^#[0-9A-F]{3}$)/i.test(color1);
		if (!isValidColor) {	color1 = '#000000'	}
		var isValidColor2 = /(^#[0-9A-F]{6}$)|(^#[0-9A-F]{3}$)/i.test(color2);
		if (!isValidColor2) {	color2 = '#000000'	}

		var context = this._context;
		var grad = context.createRadialGradient(cntr[0], cntr[1], 60, cntr[0]+20, cntr[1], 200);
		...
		context.beginPath();
		context.moveTo(cntr[0]+points[0][0], cntr[1]+points[0][1]);
		for(var i = 1; i < points.length; i++)
			context.lineTo(cntr[0]+points[i][0], cntr[1]+points[i][1]);
	...
	Bitmap.prototype.radialgradientFillRect = function(x1, y1, r1, r2, color1, color2, flicker, brightness, direction) {
		...
		if (clip == false) {
			...
			this._setDirty();
			/*
			var arr = [[200,0],[250,0],[320,300],[130,300]];
			var cntr = [225, 100];
			this.FillShape(cntr, arr, '#FF0088','#00FF00',0);
			*/
		}
	};
	...
	// Direction 인자 삭제 20200722 staticccast
	Bitmap.prototype.radialgradientFillRect2 = function(x1, y1, r1, r2, color1, color2, flashlength, flashwidth) {
		x1=x1+20;
		...
		// 마우스 커서 방향 손전등 기능
		// 미세 조정을 위해 상수 값 수정, 혹은 에디터 내의 상수 값 수정
		// 2020.02.04. staticccast 
			var playerX = $gamePlayer.screenX();
			var playerY = $gamePlayer.screenY();
			var theta = Math.atan2(TouchInput._y - playerY, TouchInput._x - playerX);
			x1 = playerX + cone*40*Math.cos(theta) + 24;
			y1 = playerY + cone*40*Math.sin(theta) - 30;
