...
Scene_Title.prototype.createCommandWindow = function() {
    ...
    this._commandWindow.setHandler('gameEndFreal', this.commandEndGame.bind(this));
    //타이틀에서 게임종료 기능 추가 2020.01.07. staticccast
    ...
    if (TouchInput.isTriggered() || this._touchCount > 0) {
        if (TouchInput.isPressed()) {
            if (this._touchCount === 0 || this._touchCount >= 15) {
                ...
                /*
                    클릭시 자동 (달리기로 최적 경로) 이동 기능 삭제
                    난이도가 과하게 쉬워지고 생생함을 해치므로
                    2020.02.01 staticccast
                */
            }
            this._touchCount++;
        } else {
...
Scene_Map.prototype.updateScene = function() {
    ...
    /* 
    (맵에서 C 버튼으로 호출하는)
    게임 종료 메뉴 기능 : Scene update 문
    2020.02.01. staticccast
    */
        this.updateCallEndMenu();
    }
...
Scene_Map.prototype.isEndMenuCalled = function(){
/*
    게임 종료 메뉴 호출부
    새로운 버튼을 할당
    모바일 경우 어떻게 할지 생각해봐야
...
*/
    return Input.isTriggered('endgamemenu');
    // isTriggered || 터치 input 식으로 구현해야
    // 터치 어떻게 다룰지 고민해봐야
};

Scene_Map.prototype.callEndMenu = function() {
...
Scene_Map.prototype.updateCallEndMenu = function() {
    if (this.isEndMenuCalled()) {
...
Scene_Map.prototype.callMenu = function() {
    ...
    // Scene_Menu가 아닌 Scene_Item을 바로 호출
...
Scene_Menu.prototype.create = function() {
    ...
    //메뉴(가방) 호출시 소지금 표시 해제 2020.01.06. staticccast

    //this.createStatusWindow();
    //메뉴(가방) 호출시 파티원 상태 표시창(우측의 가장 큰) 표시 해제
    //바로 하단의 this._statusWindow.refresh();와 연동되므로 주의
    //2020.01.06. staticccast
};
...
Scene_Menu.prototype.start = function() {
    ...
    //상단의 this.createStatusWindow(); 와 연동됨
};
...
Scene_ItemBase.prototype.determineItem = function() {
    /*
        아이템 사용시 액터(파티) 창 뜨게 하는 부분
        자동으로 파티멤버 member[0] 에게 사용되게 만들기
        member[0]은 반드시 null이 아니므로
        superclass를 수정해도 큰 문제 없을 것
        2020.01.14. staticccast
    */
...
Scene_Item.prototype.create = function() {
    ...
    //아이템 설명 창 부활 2020.02.23. staticccast
...
Scene_Item.prototype.createCategoryWindow = function() {
    ...
    this._categoryWindow.y = this._helpWindow.height + 37;
    //원본 this._categoryWindow.y = this._helpWindow.height;
    //아이템 카테고리 창과 설명 창 위치가 서로 종속적 2020.03.03. staticccast
...
// 심플 아이템창 호출 2020.01.06. staticccast
 {
    var wid = 500;  // 아이템 창 가로
    var hgt = 300;  // 아이템 창 세로

    this._itemWindow = new Window_ItemList(
        (Graphics.boxWidth - wid)/2, ((Graphics.boxHeight - hgt)/2)+44, wid, hgt);
...
/*
    저장/불러오기 시스템 : 자동저장 데이터 보호
    2020.05.30. staticccast

    추후에
    덮어쓰기시 YES/NO 선택지 (setHandler 이용) 기능 추가할 것
*/
...
Scene_GameEnd.prototype.createCommandWindow = function() {
    ...
    this._commandWindow.setHandler('gameEndFreal', this.gameEndFreal.bind(this));
    // 실제로 게임 종료하는 기능 추가
...
Scene_GameEnd.prototype.gameEndFreal = function() {
// 실제로 게임 종료하는 기능 추가 2020.01.06. staticccast
// 정밀 검사 안 해보았으나 (작업관리자 상에선) 메모리 누수 없는 것으로 보임
    window.close();
}
